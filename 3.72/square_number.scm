...
(define (find-series-length-three s)
  (let ((scar (stream-car s))
        (scadr (stream-car (stream-cdr s)))
        (scaddr (stream-car (stream-cdr (stream-cdr s)))))
    (if (= scar scadr scaddr)
        (cons-stream scar
                     (find-series-length-three (stream-cdr s)))
        (find-series-length-three (stream-cdr s)))))

(define s (find-series-length-three ordered-pow-sum))

(display-partial-stream s 5)
