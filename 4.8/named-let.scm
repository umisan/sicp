...
(define (let-name-definitions exp) (caddr exp))

(define (let-name-body exp) (cdddr exp))

(define (make-named-function exp)
  (cons 'define 
        (cons 
          (list (let-name exp) (map car (let-name-definitions exp)))
          (let-name-body exp))))

(define (let->combination exp)
  (if (named-let? exp)  
      (let ((parameters (map car (let-name-definitions exp)))
            (expressions (map cadr (let-name-definitions exp))))
        (cons
          (make-lambda parameters (cons (make-named-function exp) (let-name-body exp)))
          expressions))
      (let ((paramenters (map car (let-definitions exp)))
            (expressions (map cadr (let-definitions exp))))
        (cons 
          (make-lambda paramenters (let-body exp))
          expressions))))
...
(define (let*->nested-lets exp)
  (define (nest definitions body)
    (if (null? definitions)
        (sequence->exp body)
        (make-let
          (list (caar definitions))
          (list (nest (cdr definitions) body)))))
  (nest (let*-definitions exp) (let*-body exp)))
