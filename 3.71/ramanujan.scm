...
(define cubic-sum-pairs (weighted-pairs integers integers (lambda (x) (cubic-sum (car x) (cadr x)))))

(define cubic-sum-pairs-weight (stream-map (lambda (x) (cubic-sum (car x) (cadr x))) cubic-sum-pairs))

(define (find-same-value s)
  (let ((scar (stream-car s))
        (scdr (stream-car (stream-cdr s))))
    (if (= scar scdr)
        (cons-stream scar
                     (find-same-value (stream-cdr s)))
        (find-same-value (stream-cdr s)))))

(define ramanujan (find-same-value cubic-sum-pairs-weight))

(display-partial-stream ramanujan 6)
