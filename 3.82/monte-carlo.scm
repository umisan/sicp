...
(define (rand-update x) (remainder (+ x 1812433253) 4294967296))

(define (make-rand)
  (define (generate) random-numbers)
  (define (reset new-value)
    (generate-random-numbers new-value))
  (define (dispatch m)
...
(define (monte-carlo2 experiment)
  (define (expand pass total)
    (cons-stream (cons pass total)
                 (cond ((experiment)
                        (expand (+ pass 1) (+ total 1)))
                       (else
                         (expand pass (+ total 1))))))
  (expand (if (experiment) 1 0) 1))
...
(define (experiment-series p x1 x2 y1 y2) (monte-carlo2
                            (lambda ()
                              (let ((random-x (random-in-range x1 x2))
                                    (random-y (random-in-range y1 y2)))
                                (p random-x random-y)))))

(define (estimate-integral2 p x1 x2 y1 y2)
  (scale-stream (stream-map
                  (lambda (pair) (/ (car pair) (cdr pair)))
                  (experiment-series p x1 x2 y1 y2))
    (/ (area x1 x2 y1 y2) 9.0)))

;(display-partial-stream (estimate-integral2 circle-p 2 8 4 10) 10)
;(print (/ (estimate-integral circle-p 2 8 4 10 10.0) 9.0))
(display-partial-stream (estimate-integral2 circle-p 2 8 4 10) 10)
